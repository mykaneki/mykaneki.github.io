...
![image-20230107150738940](../img/202301071507148.png)
...
- ![image-20230107150819169](../img/202301071508317.png)
- 优点
...
- ![](../img/202301071509147.png)![image-20230107150843348](C:\Users\27951\AppData\Roaming\Typora\typora-user-images\image-20230107150843348.png)
...
- UNIX的分层体系结构
  ...
  - ![image-20230107151437547](../img/202301071514722.png)
...
- UNIX内核结构
  ...
  ![image-20230107151450180](../img/202301071514355.png)
...
![image-20230107151538117](../img/202301071515523.png)
...
![image-20230107152116537](../img/202301071521734.png)
...
![image-20230107152144842](../img/202301071521040.png)
...
1. 进程上下文
   ...
   >   - 与该进程有关的各种寄存器和堆栈的值
   > - ![进程上下文结构](../img/202203290847564.png)
...
>
>      > ![image-20220404195338628](../img/202204041953735.png)
>
...
>
>    ![多级反馈队列调度](../img/202204052133986.png)
...
![UNIX 进程的状态转换](../img/202204052212093.png)
...
2. 多生产者多消费者问题
    ...
    >
    > ![image-20220506205301523](../img/202205062053845.png)
    >
    > ![image-20220506205327857](../img/202205062053143.png)
...
![image-20220630110059255](../img/image-20220630110059255.png)
...
- 管程由4部分构成
  ...
  - 对共享数据赋初值的语句
    ...
    > ![image-20220507120723091](../img/202205071207377.png)
...
![image-20220507125739878](../img/202205071258083.png)
...
![image-20220507130129519](../img/202205071301650.png)
...
![image-20220630131904284](../img/image-20220630131904284.png)
...
1. 由应用程序控制的：**覆盖**方式
   ...
   > 分为固定区和覆盖区
   >
   > ![image-20220630181746920](../img/image-20220630181746920.png)
   >
   > 对用户不透明，不能实现虚拟存储器
...
![image-20220630183529299](../img/image-20220630183529299.png)
...
2. 动态分区的分配算法
   ...
   ![image-20220630185320587](../img/image-20220630185320587.png)

3. 动态分区的回收算法

   ![image-20220630185705624](../img/image-20220630185705624.png)
...
![image-20220702111317440](../img/image-20220702111317440.png)
...
- 分配策略
  ...
  > ![image-20220702111936243](../img/image-20220702111936243.png)
...
2. 记录式的有结构文件
   ...
   > - ![image-20220702130120991](../img/image-20220702130120991.png)
...
- 文件物理结构和组织方法：顺序文件、连接文件、索引文件和直接文件
  ...
  1. 顺序文件
     ...
     > ![image-20220702131654210](../img/image-20220702131654210.png)
     >
     > - 逻辑上连续的信息物理上也连续
     > - 优点
     >   1. 存取速度快
  ...
  3. 索引文件
     ...
     > ![image-20220702131927046](../img/image-20220702131927046.png) ![image-20220702132243301](../img/image-20220702132243301.png)
     >
     > - 优点：
     >   1. 动态增长
     >   2. 增删容易
...
1. 存储设备
   ...
   2. 直接存取设备（磁盘）
      ...
      ![存取设备、物理结构和存取方法间的关系](../img/image-20220702143535760.png)
...
- 文件控制块是按名存取的前提
- ![image-20220702144531308](../img/image-20220702144531308.png)

### 5.3.1 一级目录结构

![image-20220702145102572](../img/image-20220702145102572.png)

### 5.3.2 二级目录结构

![image-20220702145120566](../img/image-20220702145120566.png)

### 5.3.3 树形目录结构

![image-20220702145141778](../img/image-20220702145141778.png)
...
![image-20220702145627362](../img/image-20220702145627362.png)

### 5.3.5 索引结点（FCB的改进）

![image-20220702145849555](../img/image-20220702145849555.png)
...
1. 系统目录实现方式
   ...
   > - 用户通过全路径名共享地访问共享文件
   >
   > - ![image-20220702150921767](../img/image-20220702150921767.png)
   >
   > - 缺点
   >
...
2. 连接实现方式（是基于索引节点的共享方式）
   ...
   > ![image-20220702150320240](../img/image-20220702150320240.png)
   >
...
1. 存取控制矩阵
   ...
   > - ![image-20220702151512223](../img/image-20220702151512223.png)

2. 存取控制表

   > 分组管理
   >
   > ![image-20220702152137088](../img/image-20220702152137088.png)
...
![文件系统的层次模型](../img/image-20220702153454726.png)
...
![image-20220703231124698](../img/image-20220703231124698.png)
...
> 保护进程的PCB（PCB的内容）
>
> ![image-20220427200753099](../img/202204272007556.png)
...
2. 三个进程从开始到全部结束完成时 CPU 的利用率是多少？I/O 利用率是多少？
   ...
   > ![img](../img/202204272359353.png)

## 22. 采用最短周期优先调度算法，回答以下问题

假定有一组作业（或进程），它们提交时间及要求运行的时间如下表所示（单位为小时， 并以十进制计）

![image-20220427194312013](../img/202204271943079.png)
...
![image-20220427194438076](../img/202204271944151.png)
...
![image-20220427194523549](../img/202204271945663.png)
...
2. 引起进程相互制约的 2 类原因是：<u>直接制约</u> 和 <u>间接制约</u>。
   ...
   > ![image-20220420130940896](../img/202204201309971.png)
   >
...
3. 为实现消息缓冲通信，在 PCB 中应增加 <u>mq、mutex 和 sm</u> 三个数据项。
   ...
   > ![image-20220420131415826](../img/202204201314885.png)
...
![image-20220421094557754](../img/202204210946900.png)![image-20220421094613987](../img/202204210946097.png)

![image-20220421094650817](../img/202204210946923.png)
